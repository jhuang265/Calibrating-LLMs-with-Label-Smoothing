...
def _cce_smooth_backward_kernel(
    E,
    C,
    LSE,
    dOut,
    ...
    if HAS_TARGETS:
        ...
        if HAS_SMOOTHING:
            d_accum += tl.where(is_target, -(1 - smoothing + smoothing / V), -smoothing / V)
        else:
...
_cce_smooth_backward_kernel = triton.jit(_cce_smooth_backward_kernel)
_cce_smooth_backward_kernel = triton.heuristics(  # type: ignore
    {
...
)(_cce_smooth_backward_kernel)
_cce_smooth_backward_kernel = cce_backward_autotune()(_cce_smooth_backward_kernel)  # type: ignore


def cce_smooth_backward_kernel(
    do: torch.Tensor,
    e: torch.Tensor,
    c: torch.Tensor,
    lse: torch.Tensor,
    ...
    _cce_smooth_backward_kernel[grid](
        e,
        c,
        lse,
        do,
