    ...
    do
        ...
        do
            ...
            do
                ...
                if [ ! -f /work/jerry/output-openhermes/$reduction/$learning_rate/$NAME-$smoothing/config.json ]; then
                    accelerate launch \
                    ...
                    --output_dir=/work/jerry/output-openhermes/$reduction/$learning_rate/$NAME-$smoothing \
                    --dataset_mix_dir=/work/jerry/output-openhermes/$NAME \
                    --dataset_mixer='{"teknium/OpenHermes-2.5": 1.0}' \
                    --reduce_loss=$reduction
                else
                    echo "File '/work/jerry/output-openhermes/$reduction/$learning_rate/$NAME-$smoothing/config.json' already exists."
                fi
            done
