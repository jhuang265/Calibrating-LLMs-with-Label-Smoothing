...
do
    ...
    do
        ...
        do
            for softcap in 32 48 64 96 128;
            do
                # for config in \
                ...
                do
                    ...
                    if [ ! -f /work/jerry/output-softcap-openhermes/$softcap/$reduction/$learning_rate/$NAME-$smoothing/config.json ]; then
                        accelerate launch \
                        ...
                        --softcap=$softcap \
                        --output_dir=/work/jerry/output-softcap-openhermes/$softcap/$reduction/$learning_rate/$NAME-$smoothing \
                        --dataset_mix_dir=/work/jerry/output-softcap-openhermes/$NAME \
                        --dataset_mixer='{"teknium/OpenHermes-2.5": 1.0}' \
                        --reduce_loss=$reduction
                    else
                        echo "File '/work/jerry/output-softcap-openhermes/$softcap/$reduction/$learning_rate/$NAME-$smoothing/config.json' already exists."
                    fi
                done
