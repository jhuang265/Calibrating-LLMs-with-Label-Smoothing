...
class FlatArguments:
    ...
    label_smoothing: float = field(
        default=0.0,
        metadata={"help": "The amount of label smoothing to use for computing losses. Defaults to 0 (no label smoothing)."}
    )
    softcap: float = field(
        default=None,
        metadata={"help": "Softcapping."}
    )
    temperature: float = field(
        default=None,
        metadata={"help": "Temperature for scaling."}
    )
...
def main(args: FlatArguments):
    ...
    if args.dataset_mixer is not None:
        if isinstance(args.dataset_mixer, str):
            print(args.dataset_mixer)
            args.dataset_mixer = yaml.safe_load(args.dataset_mixer)
        args.dataset_mixer_list = [str(item) for pair in args.dataset_mixer.items() for item in pair]
        print(args.dataset_mixer_list)
    ...
    if args.model_name_or_path:
        if args.use_qlora:
            ...
            model = AutoLigerKernelForCausalLM.from_pretrained(
                ...
                use_flash_attention_2=True if (args.use_flash_attn or not "gemma2" in args.model_name_or_path) else False,
            ...
            model = AutoModelForCausalLM.from_pretrained(
                ...
                low_cpu_mem_usage=args.low_cpu_mem_usage,
                torch_dtype=torch.bfloat16,
                attn_implementation="flash_attention_2" if (args.use_flash_attn or not "gemma2" in args.model_name_or_path) else "eager",
            )
    else:
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                if args.reduce_loss == "mean":
                    ...
                    # Flatten the tokens
                    # loss_fct = torch.nn.CrossEntropyLoss(reduction="mean", label_smoothing=args.label_smoothing)
