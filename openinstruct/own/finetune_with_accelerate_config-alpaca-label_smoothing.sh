    ...
    do
        ...
        do
            ...
            do
                ...
                if [ ! -f /work/jerry/output-alpaca/$reduction/$learning_rate/$NAME-$smoothing/config.json ]; then
                    accelerate launch \
                    ...
                    --output_dir=/work/jerry/output-alpaca/$reduction/$learning_rate/$NAME-$smoothing \
                    --dataset_mix_dir=/work/jerry/output-alpaca/$NAME \
                    --dataset_mixer='{"arazd/tulu_stanford_alpaca": 1.0}' \
                    --reduce_loss=$reduction
                else
                    echo "File '/work/jerry/output-alpaca/$reduction/$learning_rate/$NAME-$smoothing/config.json' already exists."
                fi
            done
